...
fileset_pick(fileset_t *fileset, int flags, int tid, int index)
{
	...
	filebench_log(LOG_DEBUG_SCRIPT, "Picked file %s/%s", 
			entry->fse_parent->fse_path,entry->fse_path);
	return (entry);
...
//printf("debug jwq: preallocated :%d ,%s, %s,\n", preallocated, __func__, __FILE__);
		if (reusing)
