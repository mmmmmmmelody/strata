...
#include "ipc.h"
#include "eventgen.h"
#include "../../libfs/src/mlfs/mlfs_interface.h"
...
procflow_createproc(procflow_t *procflow)
{
	...
	char dev_id[5];
	sprintf(dev_id, "%d", procflow->mlfs_dev_id);
	setenv("DEV_ID", dev_id, 1);
	...
	if (pid == 0) {
		...
		procflow->pf_pid = pid;
		printf("Parent process with pid %d spawned child\n", procflow->pf_pid);
		printf("Parent process set env DEV_ID to: %s\n",getenv("DEV_ID"));
	}
...
procflow_exec(char *name, int instance)
{
	...
	if (0) {
		...
		shell_debug = getenv("MLFS_DEBUG");

		if (shell_debug) {
			debug = 1;
			//kill(syscall(__NR_gettid), SIGSTOP);
			kill(getpid(), SIGSTOP);
			asm("int $3");
		}
	}
...
procflow_shutdown(void)
{
	...
	// This is master information
	if (mlfs) {
		printf("[master] shuting down mlfs\n");
		shutdown_fs();
	}
}
