...
{
if((inode->inum) > 60000)
mlfs_info("debug jwq %s inode_num not normal\n", "read_extent_tree_block");
...
static int mlfs_ext_check(struct inode *inode, struct mlfs_extent_header *eh,
		int depth, mlfs_fsblk_t pblk) 
{
mlfs_assert(inode->inum < 60000);
	struct mlfs_extent_tail *tail;
	const char *error_msg;
	int max = 0;
	if (le16_to_cpu(eh->eh_magic) != MLFS_EXT_MAGIC) {
		error_msg = "invalid magic";
char debugj[128];
sprintf(debugj, "debug jwq: %hx, %hx %u %u %u", le16_to_cpu(eh->eh_magic), MLFS_EXT_MAGIC, inode->dev, inode->itype, inode->inum);
mlfs_info("%s\n", debugj);
		goto corrupted;
	}
